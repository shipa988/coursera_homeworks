...
/*func NewProfileParams(login string) *ProfileParams {
	var _ProfileParams= new(ProfileParams)
	_ProfileParams.Login=login
	return _ProfileParams
}

func NewCreateParams(login,name,status string,age int) *CreateParams {
	var _CreateParams= new(CreateParams)
	_CreateParams.Login=login

	_CreateParams.Name=name
	_CreateParams.Age=age
	if strings.Contains("user|moderator|admin",status,)
